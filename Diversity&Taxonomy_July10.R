...
taxonkit_df <- read.delim(
  text = paste(taxonkit_out, collapse = "\n"),
  sep = "\t",
  header = FALSE,
...
remove_species_BLAST <- function(df, species_vec, genus_vec) {
  ...
  removed_df <- df %>%
    filter(sscinames %in% species_vec | genus %in% genus_vec)
...
# 349 commensal ASVs were identified and removed from our ASV list for diversity analysis

diversity_matrix_MC_ASV <- seqtab_12SV5_MC_long_postdecon_filtered %>%
  filter(!fasta_id %in% commensal_fasta_id)

n_distinct(diversity_matrix_MC_ASV$fasta_id) # 2241 unique fasta_ids 

#====Species Lists for each Classifier====

n_distinct(BLAST_assignments_postdecon_filtered$sscinames) # 49 (remove NA)
species_list_BLAST <- 
  as.list(unique(BLAST_assignments_postdecon_filtered$sscinames))
...
ggplot(asv_counts_rep_merged,
  ...
  labs(x = "Sample", y = "ASV Number",
       title = "ASV Abundance - Per Replicate",
       fill = "FASTA ID") +
  theme_bw() +
...
ggplot(diversity_matrix_MC_ASV_taxa_normalized_fasta, 
  ...
  theme(axis.text.x = element_text(angle = 90),legend.position = "upper") + 
...
ggplot(diversity_matrix_MC_ASV_taxa_normalized_species, 
  ...
  scale_fill_manual(values = species_colors) +
  labs(title = "Normalized Species Abundance - Mowrow Creek 12SV5")+
  geom_bar(stat = "identity", colour = "white")+
...
##====6. Drop any all-zero species columns====
...
ggplot(data = data.scores_MC, aes(x = NMDS1, y = NMDS2)) +
  ...
  labs(shape = "Site", linetype = "Site", title = "Raup-Crick based NMDS Evaluation of ASV Presence/Absence in Mowrow Creek") +
...
ggplot(alpha_12SV5_MC_All_metadata_fasta, 
  ...
  theme_bw()+
  labs(
    title = "Mowrow Creek Shannon Alpha Diversity by SubLocation",
    x = "SubLocation",
    y = "Shannon Diversity Index"
  ) +
  theme(
...
ggplot(alpha_12SV5_MC_All_metadata_fasta, 
  ...
  theme_bw()+
  labs(
    title = "Mowrow Creek Simpson's Diversity by SubLocation",
    x = "SubLocation",
...
ggplot(alpha_12SV5_MC_All_metadata_fasta, 
  ...
  theme_bw()+
  labs(
    title = "Mowrow Creek Richness by SubLocation",
    x = "SubLocation",
...
ggplot(alpha_12SV5_MC_All_metadata_fasta, 
  ...
  theme_bw()+
  labs(
    title = "Mowrow Creek Evenness by SubLocation",
    x = "SubLocation",
    y = "Evenness"
  ) +
  theme(
...
ggiNEXT(out_sub_incidence, type=1, facet.var="Assemblage") +
  ...
  xlab("Species Counts") +
  labs(title = "Incidence-based Species Accumulation") +
