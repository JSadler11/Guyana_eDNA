...
#
# This section re-loads the WP2A filtered CSV and applies the following
...
ggplot(seqtab_12SV5_tax_long_meta_clean_filtered, aes(x = SampleSite_time, fill = species, y = reads)) + 
...
#ggplot(data=seqtab_12SV5_tax_long_meta_clean_sum_reads, aes(x=Days_sum, y=reads_sum,fill=species_sum))+
...
#seqtab_12SV5_Venn<-seqtab_12SV5_tax_long_meta_clean_filtered[!(seqtab_12SV5_tax_long_meta_clean_filtered$reads==0),]
#hist(seqtab_12SV5_Venn$reads, breaks=1000,xlim=c(0,50000))
#seqtab_12SV5_Venn <- merge(lofresh_metadata_WP2_3_summary[, c("SampleSite_time", "river_section")],seqtab_12SV5_Venn, by="SampleSite_time")

#seqtab_12SV5_Venn_unique<-seqtab_12SV5_Venn
#seqtab_12SV5_Venn_unique$SampleSite_time<-NULL
#seqtab_12SV5_Venn_unique$reads<-NULL
#seqtab_12SV5_Venn_unique<-unique(seqtab_12SV5_Venn_unique)


#upper_Venn<-seqtab_12SV5_Venn %>% filter(river_section == "upper")
#upper_Venn<-upper_Venn$species

#lower_Venn<-seqtab_12SV5_Venn %>% filter(river_section == "lower")
#lower_Venn<-lower_Venn$species

#middle_Venn<-seqtab_12SV5_Venn %>% filter(river_section == "middle")
...
#venn diagram season
#seqtab_12SV5_Venn<-seqtab_12SV5_tax_long_meta_clean_filtered[!(seqtab_12SV5_tax_long_meta_clean_filtered$reads==0),]
#seqtab_12SV5_Venn <- merge(lofresh_metadata_WP2_3_summary[, c("SampleSite_time", "season")],seqtab_12SV5_Venn, by="SampleSite_time")

#Autumn_Venn<-seqtab_12SV5_Venn %>% filter(season == "Autumn")
#Autumn_Venn<-Autumn_Venn$species

#Winter_Venn<-seqtab_12SV5_Venn %>% filter(season == "Winter")
#Winter_Venn<-Winter_Venn$species

#Spring_Venn<-seqtab_12SV5_Venn %>% filter(season == "Spring")
#Spring_Venn<-Spring_Venn$species

#Summer_Venn<-seqtab_12SV5_Venn %>% filter(season == "Summer")
...
# --- Per-phylum PERMANOVA / NMDS / CCA: Rotifera --------------------
# Identical workflow to the Arthropoda block above.
# ---------------------------------------------------------------------
...
adon.results_WP2_Rotifera<-adonis2(adonis_data_Rotifera ~ adonis_data_Rotifera_meta$Dist_from_lake+
                                       adonis_data_Rotifera_meta$season+
                                     adonis_data_Rotifera_meta$Days+
...
# --- Per-phylum PERMANOVA / NMDS / CCA: Nematoda --------------------
# Identical workflow to the Arthropoda block above.
# ---------------------------------------------------------------------
...
adon.results_WP2_Nematoda<-adonis2(adonis_data_Nematoda ~ adonis_data_Nematoda_meta$Dist_from_lake+
                                     adonis_data_Nematoda_meta$season+
                                     adonis_data_Nematoda_meta$Days+
...
# --- Per-phylum PERMANOVA / NMDS / CCA: Annelida --------------------
# Identical workflow to the Arthropoda block above.
# ---------------------------------------------------------------------
...
adon.results_WP2_Annelida<-adonis2(adonis_data_Annelida ~ adonis_data_Annelida_meta$Dist_from_lake+ 
                                     adonis_data_Annelida_meta$season+
                                     adonis_data_Annelida_meta$Days+
...
seqtab_12SV5_tax_normalised_wide_Rotifera <- seqtab_12SV5_tax_normalised_wide[ , grepl( "Rotifera" , names( seqtab_12SV5_tax_normalised_wide ) ) ]
shannon_index_Rotifera<-diversity(seqtab_12SV5_tax_normalised_wide_Rotifera, index = "shannon")
...
ggplot(shannon_index_Rotifera_meta_data, aes(pH, shannon_index)) + 
  geom_point() + 
  geom_smooth(method = "gam", formula = y ~ s(x))+theme_bw()+
  theme(legend.position="none")


mod_gam_Rotifera <- mgcv::gam(shannon_index ~ 
                                season+
                                s(Dist_from_lake)+
                                s(Days)+
                                s(Dist_from_lake, by=season)+
...
summary(mod_gam_Rotifera)
anova(mod_gam_Rotifera)

mod_gam_Rotifera_summary<-summary(mod_gam_Rotifera)
mod_gam_Rotifera_summary<-mod_gam_Rotifera_summary[["s.table"]]
mod_gam_Rotifera_summary<-data.frame(mod_gam_Rotifera_summary)
mod_gam_Rotifera_summary$edf<-NULL
mod_gam_Rotifera_summary$Ref.df<-NULL
mod_gam_Rotifera_summary<-round(mod_gam_Rotifera_summary, digits = 2)
mod_gam_Rotifera_summary$F<-paste("F = ",mod_gam_Rotifera_summary$F,  sep = "")
mod_gam_Rotifera_summary$p.value<-paste("p = ",mod_gam_Rotifera_summary$p.value,  sep = "")

mod_gam_Rotifera_summary$p.value<-sub('^p = 0$', 'p < 0.01', mod_gam_Rotifera_summary$p.value)

mod_gam_Rotifera_summary$Rotifera<-paste(mod_gam_Rotifera_summary$F,mod_gam_Rotifera_summary$p.value,
                                               sep = ", ")
mod_gam_Rotifera_summary$F<-NULL
mod_gam_Rotifera_summary$p.value<-NULL

#ALPHA DIVERSITY Nematoda
seqtab_12SV5_tax_normalised_wide_Nematoda <- seqtab_12SV5_tax_normalised_wide[ , grepl( "Nematoda" , names( seqtab_12SV5_tax_normalised_wide ) ) ]
shannon_index_Nematoda<-diversity(seqtab_12SV5_tax_normalised_wide_Nematoda, index = "shannon")
...
ggplot(shannon_index_Nematoda_meta_data, aes(pH, shannon_index)) + 
     geom_point() + 
     geom_smooth(method = "gam", formula = y ~ s(x))+theme_bw()+
     theme(legend.position="none")

ggplot(shannon_index_Nematoda_meta_data, aes(temp_monthly_average, shannon_index)) + 
  geom_point() + 
  geom_smooth(method="loess", span=0.7)+theme_bw()+
  theme(legend.position="none")

ggplot(shannon_index_Nematoda_meta_data, aes(daily_flow_by_catchment, shannon_index)) + 
  geom_point() + 
  geom_smooth(method="loess", span=0.7)+theme_bw()+
  theme(legend.position="none")

ggplot(shannon_index_Nematoda_meta_data, aes(week_before_flow_by_catchment, shannon_index)) + 
  geom_point() + 
  geom_smooth(method="loess", span=0.7)+theme_bw()+
  theme(legend.position="none")

ggplot(shannon_index_Nematoda_meta_data, aes(river_gradient_percent, shannon_index)) + 
  geom_point() + 
  geom_smooth(method="loess", span=0.7)+theme_bw()+
  theme(legend.position="none")


mod_gam_Nematoda <- mgcv::gam(shannon_index ~ 
                                season+
                                s(Dist_from_lake)+
                                s(Days)+
                                s(Dist_from_lake, by=season)+
...
summary(mod_gam_Nematoda)
anova(mod_gam_Nematoda)

mod_gam_Nematoda_summary<-summary(mod_gam_Nematoda)
mod_gam_Nematoda_summary<-mod_gam_Nematoda_summary[["s.table"]]
mod_gam_Nematoda_summary<-data.frame(mod_gam_Nematoda_summary)
mod_gam_Nematoda_summary$edf<-NULL
mod_gam_Nematoda_summary$Ref.df<-NULL
mod_gam_Nematoda_summary<-round(mod_gam_Nematoda_summary, digits = 2)
mod_gam_Nematoda_summary$F<-paste("F = ",mod_gam_Nematoda_summary$F,  sep = "")
mod_gam_Nematoda_summary$p.value<-paste("p = ",mod_gam_Nematoda_summary$p.value,  sep = "")

mod_gam_Nematoda_summary$p.value<-sub('^p = 0$', 'p < 0.01', mod_gam_Nematoda_summary$p.value)

mod_gam_Nematoda_summary$Nematoda<-paste(mod_gam_Nematoda_summary$F,mod_gam_Nematoda_summary$p.value,
                                           sep = ", ")
mod_gam_Nematoda_summary$F<-NULL
mod_gam_Nematoda_summary$p.value<-NULL

#ALPHA DIVERSITY Mollusca
seqtab_12SV5_tax_normalised_wide_Mollusca <- seqtab_12SV5_tax_normalised_wide[ , grepl( "Mollusca" , names( seqtab_12SV5_tax_normalised_wide ) ) ]
shannon_index_Mollusca<-diversity(seqtab_12SV5_tax_normalised_wide_Mollusca, index = "shannon")
...
shannon_index_Mollusca_meta_data$Days<-as.numeric(shannon_index_Mollusca_meta_data$Days)
...
shannon_index_Mollusca_meta_data<- merge(lofresh_metadata_WP2_3[, c("SampleSite_time",
                                                                    "season",
                                                                    "temp_monthly_average",
                                                                    "conductivity_uS_cm",
                                                                    "gran_alk_ueq_L","daily_flow_by_catchment",
...
shannon_index_Mollusca_meta_data$Days<-as.numeric(shannon_index_Mollusca_meta_data$Days)
shannon_index_Mollusca_meta_data<-shannon_index_Mollusca_meta_data[shannon_index_Mollusca_meta_data$shannon_index != 0, ] 

shannon_index_Mollusca_meta_data$season<-as.factor(shannon_index_Mollusca_meta_data$season)

mod_gam_Mollusca <- mgcv::gam(shannon_index ~ 
                                season+
                                 s(Dist_from_lake,k=8)+
                                 s(Days),
               data=shannon_index_Mollusca_meta_data)
summary(mod_gam_Mollusca)
anova(mod_gam_Mollusca)

mod_gam_Mollusca_summary<-summary(mod_gam_Mollusca)
mod_gam_Mollusca_summary<-mod_gam_Mollusca_summary[["s.table"]]
mod_gam_Mollusca_summary<-data.frame(mod_gam_Mollusca_summary)
mod_gam_Mollusca_summary$edf<-NULL
mod_gam_Mollusca_summary$Ref.df<-NULL
mod_gam_Mollusca_summary<-round(mod_gam_Mollusca_summary, digits = 2)
mod_gam_Mollusca_summary$F<-paste("F = ",mod_gam_Mollusca_summary$F,  sep = "")
mod_gam_Mollusca_summary$p.value<-paste("p = ",mod_gam_Mollusca_summary$p.value,  sep = "")

mod_gam_Mollusca_summary$p.value<-sub('^p = 0$', 'p < 0.01', mod_gam_Mollusca_summary$p.value)

mod_gam_Mollusca_summary$Mollusca<-paste(mod_gam_Mollusca_summary$F,mod_gam_Mollusca_summary$p.value,
                                           sep = ", ")
mod_gam_Mollusca_summary$F<-NULL
mod_gam_Mollusca_summary$p.value<-NULL
...
ggplot(shannon_index_Mollusca_meta_data, aes(pH, shannon_index)) + 
  geom_point() + 
  geom_smooth(method = "gam", formula = y ~ s(x))+theme_bw()+
  theme(legend.position="none")
...
seqtab_12SV5_tax_normalised_wide_Chordata <- seqtab_12SV5_tax_normalised_wide[ , grepl( "Chordata" , names( seqtab_12SV5_tax_normalised_wide ) ) ]
shannon_index_Chordata<-diversity(seqtab_12SV5_tax_normalised_wide_Chordata, index = "shannon")
...
seqtab_12SV5_tax_normalised_wide_Annelida <- seqtab_12SV5_tax_normalised_wide[ , grepl( "Annelida" , names( seqtab_12SV5_tax_normalised_wide ) ) ]
shannon_index_Annelida<-diversity(seqtab_12SV5_tax_normalised_wide_Annelida, index = "shannon")
...
ggplot(shannon_index_Annelida_meta_data, aes(pH, shannon_index)) + 
  geom_point() + 
  geom_smooth(method = "gam", formula = y ~ s(x))+theme_bw()+
  theme(legend.position="none")

ggplot(shannon_index_Annelida_meta_data, aes(temp_monthly_average, shannon_index)) + 
  geom_point() + 
  geom_smooth(method = "gam", formula = y ~ s(x))+theme_bw()+
  theme(legend.position="none")

mod_gam_Annelida <- mgcv::gam(shannon_index ~ season+
                                s(Dist_from_lake)+
                                s(Days)+
                                s(Dist_from_lake, by=season)+
                                s(pH)+
...
summary(mod_gam_Annelida)
anova(mod_gam_Annelida)

mod_gam_Annelida_summary<-summary(mod_gam_Annelida)
mod_gam_Annelida_summary<-mod_gam_Annelida_summary[["s.table"]]
mod_gam_Annelida_summary<-data.frame(mod_gam_Annelida_summary)
mod_gam_Annelida_summary$edf<-NULL
mod_gam_Annelida_summary$Ref.df<-NULL
mod_gam_Annelida_summary<-round(mod_gam_Annelida_summary, digits = 2)
mod_gam_Annelida_summary$F<-paste("F = ",mod_gam_Annelida_summary$F,  sep = "")
mod_gam_Annelida_summary$p.value<-paste("p = ",mod_gam_Annelida_summary$p.value,  sep = "")

mod_gam_Annelida_summary$p.value<-sub('^p = 0$', 'p < 0.01', mod_gam_Annelida_summary$p.value)

mod_gam_Annelida_summary$Annelida<-paste(mod_gam_Annelida_summary$F,mod_gam_Annelida_summary$p.value,
                                           sep = ", ")

mod_gam_Annelida_summary$F<-NULL
mod_gam_Annelida_summary$p.value<-NULL
